...
export function Pagination ({
  ...
  return <DivMain>
      {maxPage > 1
        ? <Button fixedFlex={true} disabled={!prevPageEnabled} onClick={() => { setPage(page - 1); }}>
      ...
      <DivMiddle>
        {page > 1
          ? <Button disabled={!firstPageEnabled} onClick={() => { setPage(1); }}>
            <Span>1</Span>
          </Button>
          : null}
        {leftPagesLeft === 1
          ...
          : null}
        {leftPagesLeft > 1
          ? <Button onClick={() => { setPage(Math.round(page / 2)); }}>
              <Span>...</Span>
            </Button>
          : null}
        ...
        {rightPagesLeft > 1
          ? <Button onClick={() => { setPage(Math.round(page + (maxPage - page) / 2)); }}>
              <Span>...</Span>
            </Button>
          : null}
        {rightPagesLeft === 1
          ? <Button onClick={() => { setPage(maxPage - 1); }}>
          <Span>{maxPage - 1}</Span>
        </Button>
          : null}
        {page < maxPage
          ? <Button disabled={!lastPageEnabled} onClick={() => { setPage(maxPage); }}>
              <Span>{maxPage}</Span>
            </Button>
          : null}
      </DivMiddle>
      {maxPage > 1
        ? <Button fixedFlex={true} disabled={!nextPageEnabled} onClick={() => { setPage(page + 1); }}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" width="1rem" height="1rem">
